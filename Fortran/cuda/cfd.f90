...
! Main arrays
  ...
  double precision, managed, allocatable ::  psi(:,:)
  double precision, managed, allocatable ::  psitmp(:,:)
  double precision, allocatable ::  zet(:,:), zettmp(:,:)
...
!  Some auxiliary parameters and variables
  ...
  integer :: m, n, b, h, w, size_to_copy
  integer :: iter, n_t

  double precision :: tstart, tstop, ttot, titer, modvsq, hue, err
...
!  Compute the new psi based on the old one
     ...
     if (irrotational) then
        threads = dim3(n_t,n_t,1)
        blocks = dim3(ceiling(real(m)/threads%x), ceiling(real(n)/threads%y),1)
!  Call function with no vorticity
        call jacobistep<<<blocks,threads>>>(psitmp, psi, m, n)
...
!    psi(1:m, 1:n) = psitmp(1:m, 1:n)
     ...
     call copyCuda2D<<<blocks,threads>>>(psi, psitmp, m, n)
!    size_to_copy = (m+2)*(n+2)
!    istat = cudaMemcpy(psi, psitmp, size_to_copy, cudaMemcpyDeviceToDevice)
